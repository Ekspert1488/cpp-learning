...
OUTDIR   = os.path.expanduser('~/docking_H')
CSV_OUT  = os.path.join(OUTDIR, 'scores_H.csv')
JOBS     = 8
...
def load_tasks():
    """Уникальные ok-задачи из build_results.csv, у которых есть min.pdb."""
    ok = {}
    ...
    for name in sorted(ok):
        row = ok[name]
        pdb = os.path.join(FRAG, name, f'{name}.pred1.min.pdb')
        if os.path.exists(pdb):
            tasks.append((name, row['kind'], int(row['L']), row['src'], pdb))
        else:
            print('НЕТ ФАЙЛА (пропуск):', pdb)
    return tasks
...
def dock_one(t):
    ...
    if not os.path.exists(rec):
        os.symlink(RECEPTOR, rec)          # символическая ссылка: 8200 копий не нужно
    if not os.path.exists(lig):
        os.symlink(pdb, lig)
    try:
        p = subprocess.run([HDOCK, 'receptor.pdb', 'ligand.pdb', '-out', 'job.out'],
                           cwd=workdir, stdout=subprocess.DEVNULL, timeout=1800)
...
def run_docking(limit=None):
    ...
    if limit:
        tasks = tasks[:limit]
    print('задач на докинг:', len(tasks), flush=True)
    t0 = time.time()
    n_ok = n_bad = 0
...
def _blocks(lines):
    """Все подряд идущие ATOM-блоки (цепи), независимо от их количества."""
    blocks, cur = [], []
    for l in lines:
...
def rescore_one(t):
    ...
    for i in range(1, 101):
        ...
        recep_res = []          # [(resnum, x, y, z)] только EXTRA/EPITOPE
        lig_atoms = []          # [x, y, z] — все цепи лиганда (A, у ds ещё и B)
        for b in _blocks(lines):
            chain = b[0].split()[4]
            for l in b:
                p = l.split()
                if chain == 'C':
                    if int(p[5]) in EXTRA or int(p[5]) in EPITOPE:
                        recep_res.append((int(p[5]), float(p[6]), float(p[7]), float(p[8])))
                else:
...
if __name__ == '__main__':
    ...
    if mode == 'dock':
        ...
        print('использование: python3 dock_all.py dock [limit] | rescore [limit]')